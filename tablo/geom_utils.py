...
class SpatialReference(object):
    ...
    def is_valid_proj4_projection(self):
        """ :return: true if can be projected using proj4, or false if requires an external service to project """

        # only WKIDs < 33000 map to EPSG codes
        # see: http://gis.stackexchange.com/questions/18651/do-arcgis-spatialreference-object-factory-codes-correspond-with-epsg-numbers
...
class Extent(object):
    ...
    def get_negative_extent(self):
        ...
        """
        if not self.has_negative_extent():
            return None
    ...
    def fit_image_dimensions_to_extent(self, width, height, target_resolution=None):
        """
        :param target_resolution: if provided, used for calculating dimensions instead (useful for WMS client)
    ...
    def set_to_center_and_scale(self, scale, width_in_pixels, height_in_pixels):
        """
        Modifies the extent by centering on the current extent,
        and changing to the given scale and viewport's width and height.
        """
    ...
    def project_to_web_mercator(self):
        ...
        if self.spatial_reference.wkid:
            ...
            # pypoj doesn't seem to be aware of CRS entry
            src_srs = self.spatial_reference.srs.replace('CRS:84', 'EPSG:4326')
        else:
        ...
        if src_srs == 'EPSG:4326':
            if self.ymin <= -90:
                self.ymin = -85.0511  # openlayers convention
            if self.ymax >= 90:
                self.ymax = 85.0511
    ...
    def _get_projected_extent(self, src_srs, target_srs, edge_points=9):
        ...
        Geographic latitudes must be bounded to  -85.0511 <= y <= 85.0511 prior to calling this or it will fail
    ...
    def get_scale_string(self, image_width):
        """
        Method for returning a string of text showing the current extent's scale.
        It is modified to use the extent's southern latitude to mimic the ArcGIS approach to front end scale display.
        """
        ...
        scale_in_kilometers = (
            extract_significant_digits(kilometers_per_inch / self._calc_scale_factor(southern_latitude))
        )
